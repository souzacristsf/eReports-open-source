module.exports = app => {
    ...
    return {
        ...
        runQuery: (req, res) => {
            ...
            Connection.findOne(query, {connectString: 1, password:1, user:1, _id:0})
            .then(connect => {
                execQuery(app)(connect, sql).then(result => {
                    if(result.message){
                        res.status(503).json({success: false, type: 'danger', msg: result.message, data: new Date(), title:'Status da Consulta'})        
                    }
                    res.status(201).json({success: true, type: 'success', msg: 'Consulta realizada com sucesso!!! :)', data: result, title:'Status da Consulta'})                    
                })
                .catch(err => res.status(503).json({success: false, type: 'danger', msg: 'Consulta Falhou!!! :(', data: err.message, title:'Status da Consulta'}))
            })
            .catch(err => 
                res.status(503).json({success: false, type: 'danger', msg: 'Consulta Falhou!!! :(', data: err.message, title:'Status da Consulta'})
            )
