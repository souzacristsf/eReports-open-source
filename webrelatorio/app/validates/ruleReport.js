module.exports = app => {
	...
	return {
		create: (req, res, next) => {
			...
			req.assert('categoryReportID', 'categoryReportID is required').notEmpty()
		...
		unique: (req, res, next) => {
			const query = {
				categoryReportID: req.body.categoryReportID,
			...
			CategoryReport.findOne(query)
				.then(category => {
					...
					if (category) {
						res.status(409).json({ param: 'ruleReport', msg: 'rule report exists' })
					} else {
						next();
					}
				})
